...
# ---------------- converters & pāda logic (from gita_conv) ----------------
from gita_conv import (to_deva, to_iast, iast_to_deva, tokenize, syll, join,
                       split_half_padas, norm1, snap_pair, parse_verse)

def parse_verse_padas(itrans, words=None):
    return parse_verse(itrans, words)
        ...
        for ti, (t_title, t_desc, parts) in enumerate(src["themes"]):
            ...
            for pj, (p_title, p_desc, start, end) in enumerate(parts):
                ...
                for n in range(s, e + 1):
                    ...
                    pd = parse_verse_padas(v["itrans"], words=words.get(n, {}))
...
<style>
  ...
  *{box-sizing:border-box; margin:0; padding:0;}
  ...
  @media (max-width:760px){
    ...
    .crumbs{ font-size:.84rem; gap:4px; margin-bottom:14px; }
    .crumbs button{ font-size:.84rem; padding:6px 6px; }
    .grid{ gap:12px; }
...
function doSearch(){
  ...
  const nq = normTxt(q);
  const num = digitNorm(q).match(/^(\d{1,2})[.\s]+(\d{1,2})$/);
  let hits;
  if(num){
    ...
    hits = [];
    VERSES.forEach((loc, i)=>{ if(VERSE_TEXT[i].includes(nq)) hits.push(loc); });
  }
  if(hits.length === 0){
    view.innerHTML = `<div class="res-head fade-in">${esc(L('search_results'))}</div>
      <div class="res-count fade-in">${numL(0)} ${esc(L('results'))}</div>
...
function clearSearch(){
  $('#searchInput').value = '';
  state.view = 'chapters'; showChapters();
}
...
function renderCrumbs(){
  scrollViewTop();
  crumbs.innerHTML='';
  if(state.view==='welcome' || state.view==='sections' || state.view==='chapters' || state.view==='themes' || state.view==='verses') return;
  crumbs.appendChild(crumb('🏠 '+L('chapters'), ()=>showChapters(state.section||0), state.chapter==null && state.section==null));
  if(state.section != null){
    const s1=document.createElement('span'); s1.className='sep'; s1.textContent='›'; crumbs.appendChild(s1);
    crumbs.appendChild(crumb(L('tab_'+['','karma','bhakti','jnana'][state.section]), ()=>showChapters(state.section), state.chapter==null));
  }
  if(state.chapter != null){
    const ch = DATA[state.chapter];
    const s1=document.createElement('span'); s1.className='sep'; s1.textContent='›'; crumbs.appendChild(s1);
    crumbs.appendChild(crumb(T(ch.names), ()=>showThemes(state.chapter), state.theme==null));
    if(state.theme != null){
      const t = ch.themes[state.theme];
      const s2=document.createElement('span'); s2.className='sep'; s2.textContent='›'; crumbs.appendChild(s2);
      crumbs.appendChild(crumb(T(t.titles), ()=>showThemes(state.chapter), true));
    }
  }
}
