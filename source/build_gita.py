...
__FONTS__
  ...
  *{box-sizing:border-box; margin:0; padding:0;}
  ...
  /* Top-level route chooser. Same shape as .read-tab (mūla / with meaning),
     because it is the same kind of choice: one of two views of the same
     material. The two strips never share a screen — showChapters() renders
     either topTabs() or sectionTabs(), never both — so matching them costs no
     clarity and buys one button language across the whole app. Separate pills
     also wrap on their own, which the old fixed-track segmented control could
     not do without shrinking the type. */
  .top-tabs{ display:flex; gap:8px; margin:0 0 20px; flex-wrap:wrap;}
  .top-tab{ background:var(--paper); border:2px solid var(--line); color:var(--teal); font-weight:700;
            font-size:.9rem; padding:9px 20px; border-radius:999px; cursor:pointer; min-height:44px;
            font-family:"Noto Serif Devanagari", Georgia, serif; transition:.15s;}
  .top-tab:hover{ border-color:var(--saffron); background:var(--saffron-soft);}
  .top-tab.on{ background:var(--saffron); border-color:var(--saffron); color:var(--on-accent);}
  ...
  @media (max-width:760px){
    ...
    .top-tab{ flex:1 1 auto; padding:11px 14px; font-size:.9rem; }
