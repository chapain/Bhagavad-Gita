# -*- coding: utf-8 -*-
"""pada-chheda integrity check for the Gita app: reconstruct each pāda from its
split words via external sandhi and compare with the app's actual pāda text."""
import sys, os, re, json
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
from gita_conv import norm1, parse_verse
...
for ch, W, jf in CHS:
    ...
    for vno, vd in sorted(W.items()):
        pd = parse_verse(d["verses"][str(vno)]["itrans"], words=vd)
        ptxt = [p["t"].strip() for p in pd["padas"]]
        for p in (0, 1, 2, 3):
